...
export async function fetchSiteBySlug(slug: string): Promise<Site> {
  const res = await fetch(`${API_BASE_URL}/sites/${slug}`, { cache: 'no-store' });
  if (!res.ok) throw new Error('Site non trouvé');
  return res.json();
}
...
export async function fetchSiteEspaces(slug: string): Promise<EspaceLocation[]> {
  const res = await fetch(`${API_BASE_URL}/sites/${slug}/espaces`, { cache: 'no-store' });
  if (!res.ok) throw new Error('Erreur chargement des espaces');
  return res.json();
}

export async function fetchContenus(): Promise<ContenuSensibilisation[]> {
  const res = await fetch(`${API_BASE_URL}/contenus`, { cache: 'no-store' });
  if (!res.ok) throw new Error('Erreur chargement des contenus');
  return res.json();
}

export async function createReservationApi(data: {
  site_id: string;
  date_visite: string;
  type_billet: string;
  quantite: number;
...
export async function createDemandeLocationApi(data: {
  espace_id: string;
  nom_demandeur: string;
  email_demandeur: string;
  telephone_demandeur: string;
  ...
  type_evenement: string;
  nombre_personnes: number;
}): Promise<{ demande: DemandeLocation }> {
  ...
  if (!res.ok) {
    const err = await res.json();
    throw new Error(err.error || 'Erreur lors de la réservation d\'espace');
  }
  return res.json();
}
